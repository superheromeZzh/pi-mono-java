...
class AnsiCodeTracker {
    ...
    void process(String ansiCode) {
        ...
        while (i < parts.length) {
            ...
            int consumed = applyExtendedColor(code, parts, i);
            if (consumed > 0) {
                i += consumed;
                continue;
            }
    ...
    // Returns the number of parts consumed when the code at {@code i} is a
    // 256-color (38/48;5;N) or true-color (38/48;2;R;G;B) prefix; 0 otherwise.
    private int applyExtendedColor(int code, String[] parts, int i) {
        if (code != 38 && code != 48) {
            return 0;
        }
        if (i + 2 < parts.length && parts[i + 1].equals("5")) {
            String colorCode = parts[i] + ";" + parts[i + 1] + ";" + parts[i + 2];
            setColor(code, colorCode);
            return 3;
        }
        if (i + 4 < parts.length && parts[i + 1].equals("2")) {
            String colorCode =
    ...
    private void setColor(int code, String colorCode) {
        if (code == 38) {
            fgColor = colorCode;
        } else {
            bgColor = colorCode;
    ...
    private void applyAttribute(int code) {
        switch (code) {
            case 0 -> reset();
            case 1 -> bold = true;
            case 2 -> dim = true;
