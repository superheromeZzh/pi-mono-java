...
find_jar() {
    local jar
    jar="$(find "$JAR_DIR" -maxdepth 1 -name 'pi-coding-agent-*.jar' -not -name '*-plain.jar' 2>/dev/null | head -1)"
    echo "$jar"
}
...
if [ "$REBUILD" = true ] || needs_build; then
    if [ "$REBUILD" = true ]; then
        echo "Rebuilding pi-coding-agent..."
    elif [ -z "$(find_jar)" ]; then
        echo "Building pi-coding-agent..."
    else
        echo "Source changed, rebuilding..."
    fi
    "$SCRIPT_DIR/gradlew" -p "$SCRIPT_DIR" :modules:pi-coding-agent:bootJar -q
    touch "$BUILD_MARKER"
fi
