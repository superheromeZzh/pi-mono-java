# pi-mono-java vs pi-mono (TypeScript) 完整对齐方案

## Context

pi-mono-java 基于 prompts.md 实现了核心框架，但与 pi-mono (TS) 在功能覆盖上差距显著。用户要求：
1. pi-ai 优先支持 **zai (智谱)、kimi-coding、minimax** Provider
2. pi-coding-agent **全部功能**对齐（含 RPC 模式等之前遗漏项）
3. 输出为 markdown 文件放在 pi-mono-java/ 下，并制定优先级
...
| **可插拔 StreamFunction** | streamFn in config | 硬依赖 PiAiService | ⚠️ 需改进 | P1 |
| **函数式消息获取** | getSteeringMessages | 仅 MessageQueue | ⚠️ 需改进 | P1 |
| Proxy 集成 | `proxy.ts` | - | ❌ 缺失 | P3 |

> pi-agent-core 整体对齐度较高，主要是灵活性改进。
...
| # | 任务 | 模块 | 复杂度 | 说明 |
|---|------|------|--------|------|
| 1 | ZAI Provider | pi-ai | 低 | 复用 OpenAICompletionsProvider + 自定义 baseUrl + zai thinking format |
| 2 | Kimi Coding Provider | pi-ai | 低 | 复用 AnthropicProvider + 自定义 baseUrl |
| 3 | MiniMax Provider (含 CN) | pi-ai | 低 | 复用 AnthropicProvider + 两个 baseUrl 变体 |
| 4 | EnvApiKeyResolver | pi-ai | 中 | 23+ Provider 环境变量映射, ADC 检测, Bedrock 凭证 |
| 5 | MessageTransformer | pi-ai | 中 | Tool Call ID 归一化, Thinking 丢弃, 孤立 ToolCall 合成 |
| 6 | LsTool | pi-coding-agent | 低 | 目录列表, LsOperations 接口 |
| 7 | Context Compaction | pi-coding-agent | 高 | Compactor + FileOperationTracker + LLM 摘要 + AgentLoop 集成 |
| 8 | Settings 管理 | pi-coding-agent | 中 | global + project 配置, 深度合并, 文件锁 |
| 9 | Auth 存储 | pi-coding-agent | 中 | API Key + OAuth 凭证, 0o600 权限, 刷新 |
| 10 | RPC 模式 | pi-coding-agent | 高 | JSONL 协议, 20+ 命令, stdin/stdout |
| 11 | Print 模式 | pi-coding-agent | 低 | text/json 输出, 退出码 |
| 12 | Session 分支/树 | pi-coding-agent | 高 | fork/tree 导航, JSONL 条目带 parent ID |
| 13 | Slash Commands | pi-coding-agent | 中 | 20 个内置命令框架 |
| 14 | Keybindings 系统 | pi-coding-agent | 中 | 可自定义快捷键, 21 个默认绑定 |

### P1 — 重要提升

| # | 任务 | 模块 | 说明 |
|---|------|------|------|
| 15 | Google Provider | pi-ai | google-shared + google + vertex |
| 16 | Mistral Provider | pi-ai | mistral-conversations API |
| 17 | Simple Options 工厂 | pi-ai | 统一构建 SimpleStreamOptions |
| 18 | 模型数据生成 | pi-ai | models.generated 机制 |
| 19 | 可插拔 StreamFunction | pi-agent-core | AgentLoop 解耦 |
| 20 | 函数式消息获取 | pi-agent-core | getSteeringMessages/getFollowUpMessages |
| 21 | Extension 系统 | pi-coding-agent | 工具/命令/钩子注册 |
| 22 | Prompt Templates | pi-coding-agent | $1/$2/$@ 参数替换 |
| 23 | HTML 导出 | pi-coding-agent | ANSI→HTML + 主题 |
| 24 | Git 集成 | pi-coding-agent | 分支名/未提交检测 |
| 25 | Model Resolver | pi-coding-agent | 模型回退/范围解析 |
| 26 | Resource Loader | pi-coding-agent | 统一资源发现 |
| 27 | Output Guard | pi-coding-agent | RPC 模式输出隔离 |
| 28 | Config Value 解析 | pi-coding-agent | 环境变量展开 |
| 29 | EditDiff 工具 | pi-coding-agent | diff 格式编辑 |
| 30 | Loader / CancellableLoader | pi-tui | 加载动画组件 |

### P2 — 扩展完善

| # | 任务 | 模块 | 说明 |
|---|------|------|------|
| 31 | Azure OpenAI Provider | pi-ai | 复用 Responses 共享 |
| 32 | xAI / Groq / OpenRouter | pi-ai | OpenAI-Compatible 通用 |
| 33 | OpenAI Codex Provider | pi-ai | OAuth + Responses 共享 |
| 34 | GitHub Copilot Provider | pi-ai | 动态 header |
| 35 | 剪贴板支持 | pi-coding-agent | OSC 52 + pbcopy/xclip |
| 36 | Image 处理 | pi-coding-agent | 缩放/EXIF/格式转换 |
| 37 | Diff 可视化 | pi-coding-agent | 彩色 side-by-side diff |
| 38 | Theme 系统 | pi-coding-agent | 40+ 可配颜色 |
| 39 | Package Manager | pi-coding-agent | npm/git 包发现 |
| 40 | Source Info | pi-coding-agent | 资源来源追踪 |
| 41 | Diagnostics | pi-coding-agent | 资源校验/冲突检测 |
| 42 | MIME 检测 | pi-coding-agent | 文件类型识别 |
| 43 | Migrations | pi-coding-agent | Session 格式迁移 |
| 44 | Footer Data | pi-coding-agent | 底栏信息显示 |
| 45 | StdinBuffer | pi-tui | 转义序列装配 |
| 46 | Autocomplete + Fuzzy | pi-tui | 文件路径补全 |
| 47 | TruncatedText + Spacer | pi-tui | 布局组件 |
| 48 | SettingsList | pi-tui | 设置列表组件 |
| 49 | Keybindings 组件 | pi-tui | 可配键绑定 |

### P3 — 后续打磨

| # | 任务 | 模块 |
|---|------|------|
| 50 | Google Gemini CLI / Antigravity | pi-ai |
| 51 | Cerebras / HuggingFace / OpenCode | pi-ai |
| 52 | Proxy 集成 | pi-agent-core |
| 53 | Image 组件 + Kitty/iTerm2 协议 | pi-tui |
| 54 | Changelog | pi-coding-agent |
| 55 | Timings | pi-coding-agent |
...
| 模块 | 已实现 | 缺失 | 对齐率 |
|------|--------|------|--------|
| pi-ai (Provider) | 4 | 19 | 17% |
| pi-ai (基础设施) | 4 | 6 | 40% |
| pi-agent-core | 9 | 3 | 75% |
| pi-tui (组件) | 9 | 6 | 60% |
| pi-tui (基础设施) | 5 | 5 | 50% |
| pi-coding-agent (工具) | 6 | 2 | 75% |
| pi-coding-agent (模式) | 2 | 2 | 50% |
| pi-coding-agent (核心系统) | 10 | 24 | 29% |
| **总计** | **49** | **67** | **42%** |
...
# RPC 模式测试
echo '{"type":"prompt","message":"hello"}' | java -jar pi-coding-agent.jar --mode rpc
```