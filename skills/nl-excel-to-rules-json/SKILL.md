...
- **严禁**在验证通过前把内容写入统一文件 `C:\Users\Jason\.openclaw\workspace\rules\rules.json`
- 必须先生成本次规则到一个 **临时文件**（例如：`C:\Users\Jason\.openclaw\workspace\rules\_candidate_rules.json`）
- 必须跑完 **方案一 + 方案二** 且都通过
- 通过后再问用户“是否保存到统一 rules.json？”
- 用户回复“保存”才允许执行合并落盘

### 最重要的硬规则（必须遵守）
- **生成阶段只允许写候选文件**：`C:\Users\Jason\.openclaw\workspace\rules\_candidate_rules.json`
...
```text
C:\Users\Jason\.openclaw\workspace\rules\_candidate_rules.json
```

2) 校验闭环（必须同时跑方案一 + 方案二；禁止只跑方案一）：

```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/verify_candidate_rules.py C:/Users/Jason/.openclaw/workspace/rules/_candidate_rules.json
```

3) 查重预演（不落盘）：

```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/merge_rules.py --dry-run C:/Users/Jason/.openclaw/workspace/rules/rules.json C:/Users/Jason/.openclaw/workspace/rules/_candidate_rules.json
```
...
```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/merge_rules.py C:/Users/Jason/.openclaw/workspace/rules/rules.json C:/Users/Jason/.openclaw/workspace/rules/_candidate_rules.json
```
...
```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/merge_rules.py C:/Users/Jason/.openclaw/workspace/rules/rules.json <path-to-newly-generated-rules.json>
```

保存前查重预演（不落盘）：

```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/merge_rules.py --dry-run C:/Users/Jason/.openclaw/workspace/rules/rules.json <path-to-newly-generated-rules.json>
```

注意：
- 确保目录 `C:/Users/Jason/.openclaw/workspace/rules/` 存在（不存在则创建）
- 合并后可选择运行校验：

```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/validate_rules_json.py C:/Users/Jason/.openclaw/workspace/rules/rules.json
```
...
### 闭环步骤（必须严格按顺序执行）
1) 先生成规则（先不要落盘到统一文件），写入临时文件（建议路径）：`C:\Users\Jason\.openclaw\workspace\rules\_candidate_rules.json`
2) 方案一：结构与自洽校验（必须）
...
```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/validate_rules_json.py <path-to-rules.json>
```
...
```bash
python C:/Users/Jason/.openclaw/workspace/skills/nl-excel-to-rules-json/scripts/verify_synthetic_rules.py <path-to-rules.json>
```
